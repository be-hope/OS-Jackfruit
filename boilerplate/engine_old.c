...
static int send_control_request(const control_request_t *req)
{
    if (req->kind == CMD_START || req->kind == CMD_RUN)
    {
        ...
        {
            char logfile[128];
            sprintf(logfile, "logs/%s.log", req->container_id);
        ...
        FILE *meta = fopen("/tmp/containers.txt", "a");
        fprintf(meta, "%s %d running\n", req->container_id, pid);
        fclose(meta);
    ...
    {
        char cmd[128];
        sprintf(cmd, "cat logs/%s.log", req->container_id);
        system(cmd);
        return 0;
    }
    ...
    {
        char cmd[128];
        sprintf(cmd, "pkill -f %s", req->container_id);
        system(cmd);
        printf("Stopped %s\n", req->container_id);
        return 0;
    }
